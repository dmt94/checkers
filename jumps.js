  ...
  makeMove(checkers) {
    checkers.currentPlayerTurn.squareClasses.forEach((squareClass) => { 
      ...
      let listenToFirstClick = (squareClass) => {
        ...
        let possibleMovesIdx = this.źookUpAllMoves(squareClass.index, checkers);
        ...
        checkers.currentPlayerTurn.squareClasses.forEach((sC) => {
          if (possibleMovesIdx.includes(sC.index)) {
            if (sC.value === checkers.otherPlayerTurn.playerValue) {
              let movesIdx = this.checkMovesAvailable(sC.index, checkers);
              
              checkers.currentPlayerTurn.pieces.forEach((piece) => {
                if (movesIdx.includes(piece.startingIndex)) {
                  checkers.currentPlayerTurn.squareClasses.forEach((sqc) => {
                    sqc.domSquareElement.addEventListener("click", () => {
                      sqc.domSquareElement.append(piece.divElem);
                      squareClass.value = null;
